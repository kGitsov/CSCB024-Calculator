    ...
    public class StringTokenizer
    {
        ...
        private TokenType TypeOfToken(string token)
        {
            ...
            if (Decimal.TryParse(token, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out tmpNum))
                return CSCB024_Calculator.TokenType.Number;

            if (operators.Contains(token))
                return CSCB024_Calculator.TokenType.Operator;

            if (token.Equals("("))
                return CSCB024_Calculator.TokenType.LeftBracket;

            if (token.Equals(")"))
                return CSCB024_Calculator.TokenType.RightBracket;

            if (token.Equals(","))
                return CSCB024_Calculator.TokenType.FunctionArgumentSeparator;
